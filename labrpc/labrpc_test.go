	...
	for ii := range nclients {
		go func(i int) {
			...
			for j := range nrpcs {
				...
				if reply != wanted {
					errMsg = fmt.Sprintf("wrong reply %v from Handler1, expecting %v", reply, wanted)
				}
				n += 1
			}
		}(ii)
...
func TestConcurrentOne(t *testing.T) {
	...
	for ii := range nrpcs {
		go func(i int) {
			...
			wanted := "handler2-" + strconv.Itoa(arg)
			if reply != wanted {
				errMsg = fmt.Sprintf("wrong reply %v from Handler2, expecting %v", reply, wanted)
				return
			}
			n += 1
		}(ii)
