...
var (
	...
	unit       struct{}    = captureSignal()
	finfo      *FrameworkInfo
)

const (
	colorInfo    string = "#FAFAFA"
	colorNeutral string = "#FFECB3"
	colorSuccess string = "#C8E6C9"
	colorFailure string = "#FFCDD2"
	colorFault   string = "#B3E5FC"
	colorUser    string = "#FFF176"
)

const (
	tagChecker   string = "$ Checker"
	tagPartition string = "$ Failure"
	tagInfo      string = "$ Test Info"
)
...
func Annotate(tag, desp, details string) {
	annotation.annotatePointColor(tag, desp, details, colorUser)
}

func AnnotateInterval(tag string, start int64, desp, details string) {
	annotation.annotateIntervalColor(tag, start, desp, details, colorUser)
}

func AnnotateContinuous(tag, desp, details string) {
	annotation.annotateContinuousColor(tag, desp, details, colorUser)
}

// Used by test framework.

func AnnotateInfo(desp, details string) {
	AnnotatePointColor(tagInfo, desp, details, colorInfo)
}

func AnnotateInfoInterval(start int64, desp, details string) {
	AnnotateIntervalColor(tagInfo, start, desp, details, colorInfo)
}
...
func AnnotateCheckerEnd(desp, details, color string) {
	...
	if ckbegin.ts == 0 {
		...
		AnnotatePointColor(tagChecker, desp, details, color)
		return
	}

	// Annotate as an interval if the begin timestamp is set.
	d := fmt.Sprintf("%s: %s", ckbegin.details, details)
	AnnotateIntervalColor(tagChecker, ckbegin.ts, desp, d, color)
...
func AnnotateCheckerSuccess(desp, details string) {
	AnnotateCheckerEnd(desp, details, colorSuccess)
}

func AnnotateCheckerFailure(desp, details string) {
	AnnotateCheckerEnd(desp, details, colorFailure)
}

func AnnotateCheckerNeutral(desp, details string) {
	AnnotateCheckerEnd(desp, details, colorNeutral)
}
...
func annotateFault() {
	...
	if slices.Equal(trues, finfo.connected) && slices.Equal(falses, finfo.crashed) {
		...
		AnnotateContinuousEnd(tagPartition)
		return
	}
	...
	text := builder.String()
	AnnotateContinuousColor(tagPartition, text, text, colorFault)
}
...
func (an *Annotation) cleanup(failed bool, end string) {
	...
	if fpath == "" {
		...
		file, err = os.OpenFile(fpath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0o644)
	}
	if err != nil {
